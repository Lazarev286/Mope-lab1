...
X2max = 60
X2max_n = 1
p_list = (0.85, 0.84, 0.81, 0.76)
RKR_MATRIX = {2: (1.73, 1.72, 1.71, 1.69),
...
AA2 = b2 / DX2
def Dispers():
    m1 = min(RKR_MATRIX, key=lambda x: abs(x - m))
    p = 0
    for ruv in (ruv1, ruv2, ruv3):
        if ruv > RKR_MATRIX[m1][0]:
            return False
        for rkr in range(len(RKR_MATRIX[m1])):
            if ruv < RKR_MATRIX[m1][rkr]:
                p = rkr
    return p_list[p]
...
print("Ruv3 =", ruv3)
print("-"*60)
print("Однорідна дисперсія:", Dispers())
print("-"*60)
print("mx1:", MX1)
print("mx2:", MX2)
print("my:", MY)
...
print("a2:", AA2)
print("-"*60)
Ynr = [round(line_equation(X1min, X2min), 4),
        round(line_equation(X1max, X2min), 4),
        round(line_equation(X1min, X2max), 4)]
print("Натуралізоване рівняння регресії:",Ynr)
print("-"*60)
if Ynr == Yavg:
...
def check():
    ...
    if NY1 == Yavg[0] and NY2 == Yavg[1] and NY3 == Yavg[2]:
        print("Значення перевірки нормованого рівняння регресії сходяться")
    else:
        print("Значення перевірки нормованого рівняння регресії не сходяться")
check()
